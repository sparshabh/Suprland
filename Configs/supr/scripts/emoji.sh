#!/usr/bin/env bash
# Suprland emoji picker (rofi) — pick an emoji, auto-copied to clipboard
source ~/.config/supr/System64/logger.sh 2>/dev/null
 
rofi_theme="$HOME/.config/rofi/configs/emoji.rasi"
msg='** note ** 👀 Click or Return to choose || Ctrl+V to Paste'
 
# kill any stray rofi instance first
pidof rofi > /dev/null && pkill rofi
 
if ! command -v wl-copy &>/dev/null; then
    log_error "wl-copy not found, cannot copy emoji to clipboard"
    notify-send "Emoji picker" "wl-copy missing" 2>/dev/null
    exit 1
fi
 
sed '1,/^# # DATA # #$/d' "$0" | \
rofi -i -dmenu -mesg "$msg" -config "$rofi_theme" -p "Emojis" | \
awk '{print $1}' | \
head -n 1 | \
tr -d '\n' | \
wl-copy
